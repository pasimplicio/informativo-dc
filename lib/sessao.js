...
/**
 * Dois tipos de token trafegam assinados com o mesmo segredo. O campo `tipo`
 * impede que um seja aceito no lugar do outro -- sem ele, um `state` (que o
 * usuario recebe no inicio do fluxo) poderia ser apresentado como cookie de
 * sessao.
 */
export const TIPO = { sessao: 'sessao', state: 'state' };

const VALIDADE = {
  [TIPO.sessao]: VALIDADE_S,
  [TIPO.state]: 600, // 10 min: tempo de ir ao Google e voltar
};

/** Gera um token assinado do tipo indicado. */
export async function assinar(dados, segredo, tipo = TIPO.sessao) {
  const carga = {
    ...dados,
    tipo,
    exp: Math.floor(Date.now() / 1000) + (VALIDADE[tipo] || VALIDADE_S),
  };
...
/**
 * Devolve a carga se o token for autentico, do tipo esperado e nao expirado;
 * senao null. A verificacao usa crypto.subtle.verify, que compara em tempo
 * constante.
 */
export async function verificar(token, segredo, tipoEsperado = TIPO.sessao) {
  ...
  try {
    ...
    if (carga.tipo !== tipoEsperado) return null;

    // So a sessao carrega identidade. Revalidar o dominio a cada requisicao faz
    // com que mudar a regra invalide sessoes ja emitidas, sem esperar a
    // expiracao. O `state` nao tem e-mail e nao passa por aqui.
    if (tipoEsperado === TIPO.sessao && !ehDominioPermitido(carga.email)) return null;
